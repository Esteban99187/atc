<?php
   ...
   if(array_key_exists(txtoperacion,$_POST))
   {
	   ...
	   $lobjbusqueda= new clsRep_asignacion(); 
   ...
   if ($lsOperacion=="buscar")
   {
	 $_SESSION['matriz']=$lobjbusqueda->fBusqueda();
	  ...
	  $lsbusqueda=$lobjbusqueda->fGetsbusqueda();
			...
			header("location: ../vista/visRep_asignacion.php?lsOperacion=$lsOperacion&liHay=$liHay&lsbusqueda=$lsbusqueda");
   }
   
   
   if ($lsOperacion!="buscar"){
	   
		header("location: ../vista/visRep_asignacion.php?lsOperacion=$lsOperacion");
   }
