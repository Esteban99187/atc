<?php
   ...
   if(array_key_exists(txtoperacion,$_POST))
   {
	   ...
	   $lobjbusqueda= new clsRep_Relacion_conductor(); 
   ...
   if ($lsOperacion=="buscar")
   {
	 $_SESSION['matriz']=$lobjbusqueda->fBusqueda();
	  ...
	  $lsbusqueda=$lobjbusqueda->fGetsbusqueda();
			...
			header("location: ../vista/admin.php?url=visRep_Relacion_conductor&lsOperacion=$lsOperacion&liHay=$liHay&lsbusqueda=$lsbusqueda");
   }
   
   
   if ($lsOperacion!="buscar"){
	   
		header("location: ../vista/reporte/visRep_Relacion_conductor.php?lsOperacion=$lsOperacion");
   }
