<?php
	...
	********************************************************************************************/
	...
	$liidpais= isset($_GET['liidpais']) ? $_GET['liidpais'] : null ;
	$lsdesc_pais= isset($_GET['lsdesc_pais']) ? $_GET['lsdesc_pais'] : null ;
	$lsestatus_pai= isset($_GET['lsestatus_pai']) ? $_GET['lsestatus_pai'] : null ;
...
<script type="text/javascript">
	function BuscarAjaxPais(valor){
		...
		if(document.getElementById('est1').checked){ //si esta tildado la caja de activos
			...
			ajax.send("operacion=BusquedaAjaxPais&status=1&estado="+valor); //paso variable con estatus activo(1) y nombre

		}else if(document.getElementById('est2').checked){ //si esta tildado la caja de inactivos

			ajax.send("operacion=BusquedaAjaxPais&status=0&estado="+valor); //paso variable con estatus inactivo(0) y nombre

		}else{ //si no esta tildados ni activos ni inactivos
			ajax.send("operacion=BusquedaAjaxPais&status=Null&estado="+valor); //paso variable con estatus Nulo y nombre
		}	
	}
</script>
<div class="row">
	<div class="col-sm-12">
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<form method="post" name="fpais" id="fpais" action="../controlador/control_maestro_pais.php">
					...
					<div class="row">
						<div class="form-group col-lg-3"></div>
							<input class="form-control"  type="hidden" name="txtidpais" title="ingrese nombre descriptivo del pais"  maxlength="3" tabindex="1" value="<?php print($liidpais);?>" onblur="fperderfocusidpais();">
							<div class="col-lg-6">
								<div class="form-group has-error">
								<label >Nombre:</label>
								<input class="form-control" onBlur="cambio_mayus(this)" placeholder="Solo Ingrese Letras" type="text" name="txtdesc_pais" title="ingrese nombre pais" onkeypress="return soloLetra(event)" maxlength="50"  tabindex="2"  value="<?php print($lsdesc_pais);?>"> 
							</div>
						</div>
						...
						<center>
							...
							?>
							<button type="reset"  name="btncancelar"  class="btn btn-lg btn-primary" title="Clic para Cancelar"  onclick="cancelar('maestro_pais')">Cancelar</button>
						</center>
...
<div class="row">
	<div class="col-sm-12">
		<div class="panel panel-default">
			...
			<div class="col-md-11">
				<label>El país forma parte esencial para el registro de la solicitud; por lo tanto, es un campo obligatorio que debe ser registrado.</label>
			</div> 
		</div>
	</div> 
</div>
...
<script language="javascript">
	...
	function finicio()
	{
		lof=document.fpais;
			...
			{
				lof=document.fpais;
				lof.txtdesc_pais.disabled=true;
	...
	function fnuevo()
	{
		...
		lof.txtdesc_pais.disabled=false;
		lof.txtdesc_pais.focus();
		lof.btnnuevo.disabled=true;
		lof.btnbuscar.disabled=true;
	...
	var miPopup
		function abrepais(){ //~ luego de 'funcion' se le coloca el nombre que se le dio en el onclick de arriba
		miPopup = window.open("ayuda/ayudapais.php","solicitud","width=450px,height=870px,scrollbars=yes,toolbar=No") //~ aqui se debe colocar la direccion del archivo de la ayuda
		miPopup.focus()
	}
</script>
