<?php
	...
	require_once("../../modelo/clsRep_Relacion_conductor.php");   
	$lobjPdf=new clsFpdf();
	$lobjPdf->AliasNbPages();
	$lobjPdf->AddPage("L","Legal");
	$lobjPdf->SetMargins(20,20,20);
	$lobjPdf->SetFont("arial","B",12);
	$lobjclsRep_Relacion_conductor= new clsRep_Relacion_conductor();
	$lsbusqueda= isset($_GET['lsbusqueda']) ? $_GET['lsbusqueda'] : null;
	$lobjclsRep_Relacion_conductor->fSetsbusqueda($lsbusqueda);
	$lobjclsRep_Relacion_conductor->fBusqueda($lsbusqueda);
	$laMatriz=$lobjclsRep_Relacion_conductor->fBusqueda(); 
	
	$lobjPdf->SetY(40);
	$lobjPdf->SetMargins(10,10,0);
	$lobjPdf->Cell(0,6,utf8_decode("Listado de Relación de Unidades por Conductor"),0,1,"C");
