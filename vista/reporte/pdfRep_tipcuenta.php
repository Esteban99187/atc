<?php
  ...
  //if(!array_key_exists(nombre,$_SESSION)) 
   ...
   require_once("../../modelo/class_tipo_cuenta.php");   //se define la clase que se debe llamar (que contenga el istar con la instruccion sql debida)
   $lobjPdf=new clsFpdf();
   $lobjPdf->AliasNbPages();
   $lobjPdf->AddPage("P","Letter");
   $lobjPdf->SetMargins(30,30,30);
   $lobjPdf->SetFont("arial","B",12);
   $lobjclass_tipo_cuenta= new class_tipo_cuenta(); //se cambia de acuerdo a la case que estan llamando (si es producto ya no iria 'class_tipo_cuenta sino' 'class_producto')
   $lsa= isset($_GET['lsa']) ? $_GET['lsa'] : null;
   $lobjclass_tipo_cuenta->flistar($lsa); //se define la clase y 'flistar' es la funcion que se creo en la clase para el listado
   $laMatriz=$lobjclass_tipo_cuenta->flistar(); //se vuele a definir el parametro
													
      $lobjPdf->SetY(40);
	   $lobjPdf->SetMargins(75,30,0);
	   $lobjPdf->Cell(0,6,utf8_decode("Listado de Tipos de Cuentas de la Empresa ATC .C.A."),0,1,"C"); //titulo del pdf
