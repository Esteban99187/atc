<?php
	...
	require_once("../../modelo/clsRep_Orden_conductor.php");   
	$lobjPdf=new clsFpdf();
	$lobjPdf->AliasNbPages();
	$lobjPdf->AddPage("L","Legal");
	$lobjPdf->SetMargins(20,20,20);
	$lobjPdf->SetFont("arial","B",12);
	$lobjclsRep_Orden_conductor= new clsRep_Orden_conductor();
	$lsbusqueda= isset($_GET['lsbusqueda']) ? $_GET['lsbusqueda'] : null;
	$lobjclsRep_Orden_conductor->fSetsbusqueda($lsbusqueda);
	$lobjclsRep_Orden_conductor->fBusqueda($lsbusqueda);
	$laMatriz=$lobjclsRep_Orden_conductor->fBusqueda(); 
	
	$lobjPdf->SetY(40);
	$lobjPdf->SetMargins(20,20,20);
	$lobjPdf->Cell(0,6,utf8_decode("Listado de Orden de Carga por Conductor"),0,1,"C");
	...
	for($liI=0;$liI<count($laMatriz);$liI++)
	{
		...
		$lobjPdf->Cell(40,6,utf8_decode($laMatriz[$liI][12]).' '.($laMatriz[$liI][13]),1,0,"C");
		...
		$lobjPdf->Cell(70,6,utf8_decode($laMatriz[$liI][10]),1,0,"C");
		$lobjPdf->Cell(70,6,utf8_decode($laMatriz[$liI][11]),1,1,"C");
	}
	$lobjPdf->Output();
?>
