...
require_once("../../modelo/class_ordcar.php");
   $lobj_ordcar= new class_ordcar();
   $listado_ordcar= $lobj_ordcar->flistar();
...
<div class="container" style="margin-top:10px;">
	...
	<div class="col-md-12">
      <div class="panel panel-default">
        ...
        <div class="panel-body">
            ...
            <a href="../reporte/pdfRep_Ordencarga.php?lsa=-"><input type="button" class="btn btn-primary btn-lg" value="Imprimir        " title="Click aqui para Imprimir"></a>
            </div>
    ...
    <thead>
		...
		<tr>
        <td><?php 
						for($i=0;$i<count($listado_ordcar);$i++){
						$cedula=$listado_ordcar[$i][0];
						echo "".$cedula."<hr>"; 
							}	
						?></td>
        <td><?php 
						for($i=0;$i<count($listado_ordcar);$i++){
						$idarticulo=$listado_ordcar[$i][1];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
       <td><?php 
						for($i=0;$i<count($listado_ordcar);$i++){
						$idarticulo=$listado_ordcar[$i][2];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
        <td><?php 
						for($i=0;$i<count($listado_ordcar);$i++){
						$idarticulo=$listado_ordcar[$i][6];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
		<td><?php 
						for($i=0;$i<count($listado_ordcar);$i++){
						$idarticulo=$listado_ordcar[$i][3];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
        <td><?php 
						for($i=0;$i<count($listado_ordcar);$i++){
						$idarticulo=$listado_ordcar[$i][4];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
		 <td><?php 
						for($i=0;$i<count($listado_ordcar);$i++){
						$idarticulo=$listado_ordcar[$i][5];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
		<td><?php 
						for($i=0;$i<count($listado_ordcar);$i++){
						$idarticulo=$listado_ordcar[$i][7];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
        <td><?php 
						for($i=0;$i<count($listado_ordcar);$i++){
						$idarticulo=$listado_ordcar[$i][8];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
