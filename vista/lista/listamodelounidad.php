...
require_once("../../modelo/class_modelo_unidad.php");
   $lobj_modelo_unidad= new class_modelo_unidad();
   $listado_modelo_unidad= $lobj_modelo_unidad->flistar();
...
<div class="container" style="margin-top:10px;">
	...
	<div class="col-md-12">
      <div class="panel panel-default">
        <div class="panel-heading"><strong>Lista de Modelos de unidad</strong></div>
        <div class="panel-body">
            <div class="row">
				<div class="form-group col-lg-10"></div>
            <a href="../reporte/pdfRep_Modelounidad.php?lsa=-"><input type="button" class="btn btn-primary btn-lg" value="Imprimir        " title="Click aqui para Imprimir"></a>
            </div>
    ...
    <thead>
		...
		<tr>
        <td><?php 
						for($i=0;$i<count($listado_modelo_unidad);$i++){
						$idarticulo=$listado_modelo_unidad[$i][0];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
        <td><?php 
						for($i=0;$i<count($listado_modelo_unidad);$i++){
						$idarticulo=$listado_modelo_unidad[$i][1];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
         <td><?php 
						for($i=0;$i<count($listado_modelo_unidad);$i++){
						$idarticulo=$listado_modelo_unidad[$i][2];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
        <td><?php 
						for($i=0;$i<count($listado_modelo_unidad);$i++){
						$idarticulo=$listado_modelo_unidad[$i][4];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
