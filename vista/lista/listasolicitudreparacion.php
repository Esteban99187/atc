...
require_once("../../modelo/class_solicitud_reparacion.php");
   $lobj_solicitud_reparacion= new class_solicitud_reparacion();
   $listado_solicitud_reparacion= $lobj_solicitud_reparacion->flistar();
...
<div class="container" style="margin-top:10px;">
	...
	<div class="col-md-12">
      <div class="panel panel-default">
        <div class="panel-heading"><strong>Lista de Solicitud de Reparación de Unidades</strong></div>
        <div class="panel-body">
            <div class="row">
				<div class="form-group col-lg-10"></div>
            <a href="../reporte/pdfRep_solicitudreparacion.php?lsa=-"><input type="button" class="btn btn-primary btn-lg" value="Imprimir        " title="Click aqui para Imprimir"></a>
            </div>
    ...
    <thead>
		...
		<tr>
        <td><?php 
						for($i=0;$i<count($listado_solicitud_reparacion);$i++){
						$cedula=$listado_solicitud_reparacion[$i][0];
						echo "".$cedula."<hr>"; 
							}	
						?></td>
        <td><?php 
						for($i=0;$i<count($listado_solicitud_reparacion);$i++){
						$idarticulo=$listado_solicitud_reparacion[$i][1];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
       <td><?php 
						for($i=0;$i<count($listado_solicitud_reparacion);$i++){
						$idarticulo=$listado_solicitud_reparacion[$i][6];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
        <td><?php 
						for($i=0;$i<count($listado_solicitud_reparacion);$i++){
						$idarticulo=$listado_solicitud_reparacion[$i][7];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
		<td><?php 
						for($i=0;$i<count($listado_solicitud_reparacion);$i++){
						$idarticulo=$listado_solicitud_reparacion[$i][9];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
        <td><?php 
						for($i=0;$i<count($listado_solicitud_reparacion);$i++){
						$idarticulo=$listado_solicitud_reparacion[$i][10];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
		<td><?php 
						for($i=0;$i<count($listado_solicitud_reparacion);$i++){
						$idarticulo=$listado_solicitud_reparacion[$i][3];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
        <td><?php 
						for($i=0;$i<count($listado_solicitud_reparacion);$i++){
						$idarticulo=$listado_solicitud_reparacion[$i][4];
						echo "".$idarticulo."<hr>"; 
							}	
						?></td>
