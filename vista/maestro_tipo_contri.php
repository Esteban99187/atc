...
$liidtipo_contribuyente = isset($_GET['liidtipo_contribuyente']) ? $_GET['liidtipo_contribuyente'] : null;
$lsdesc_tipo_cont = isset($_GET['lsdesc_tipo_cont']) ? $_GET['lsdesc_tipo_cont'] : null;
$lihay = isset($_GET['lihay']) ? $_GET['lihay'] : null;
$lshacer = isset($_GET['lshacer']) ? $_GET['lshacer'] : null;
$lsestatus_tipcont= isset($_GET['lsestatus_tipcont']) ? $_GET['lsestatus_tipcont'] : null ;
$lsoperacion = isset($_GET['lsoperacion']) ? $_GET['lsoperacion'] : null;
?>
<script type="text/javascript">
	function BuscarAjaxtipocontri(valor){
		...
		if(document.getElementById('est1').checked){ //si esta tildado la caja de activos
			...
			ajax.send("operacion=BusquedaAjaxtipocontri&status=1&estado="+valor); //paso variable con estatus activo(1) y nombre

		}else if(document.getElementById('est2').checked){ //si esta tildado la caja de inactivos

			ajax.send("operacion=BusquedaAjaxtipocontri&status=0&estado="+valor); //paso variable con estatus inactivo(0) y nombre

		}else{ //si no esta tildados ni activos ni inactivos
			ajax.send("operacion=BusquedaAjaxtipocontri&status=Null&estado="+valor); //paso variable con estatus Nulo y nombre
		}	
	}
</script>
<div class="row">
	<div class="col-sm-12">
		<div class="panel panel-default">
			<div class="panel-heading"> 
				...
				<div class="panel-options">  <a href="#" data-rel="collapse"><i class="entypo-down-open"></i></a> <a href="#" data-rel="reload"><i class="entypo-arrows-ccw"></i></a> <a href="#" data-rel="close"><i class="entypo-cancel"></i></a> <i class="entypo-help" onclick="abretipo_contribuyente()"></i></div> <!-- agregar este linea depues del icono close (despues del ultimo </a>) y cambiar el nombre de la funcion onclick<i class="entypo-help" onclick="abreproducto()"></i> luego al final dentro del <script> colocar la funcion onclick -> revisar el final para ver funcion onclick-->
			</div>
				<div class="panel-body">
					<form method="post" name="fmaestro_tipo_contri" id="fmaestro_tipo_contri" action="../controlador/control_maestro_tipo_contri.php">
						...
						<div class="row">
							<div class="form-group col-lg-3"></div>
								<input class="form-control"  type="hidden" name="txtidtipo_contribuyente" onkeypress="return soloNumero(event)" title="ingrese codigo tipo contribuyente"  maxlength="3" tabindex="1" value="<?php print($liidtipo_contribuyente);?>" onblur="fperderfocusidtipo_contribuyente();">
								<div class="col-lg-6">
								<div class="form-group has-error">
									<label >Nombre:</label>
										<input class="form-control"  type="text" onBlur="cambio_mayus(this)" name="txtdesc_tipo_cont" placeholder="Solo Ingrese Letras" onkeypress="return soloLetra(event)" title="ingrese nombre tipo contribuyente" maxlength="50"  tabindex="2"  value="<?php print($lsdesc_tipo_cont);?>"> 
							</div>
							</div>
						...
						<div class="row">
							...
							?>
							<button type="reset"  name="btncancelar"  class="btn btn-lg btn-primary" title="Clic para Cancelar"  onclick="cancelar('maestro_tipo_contri')">Cancelar</button>
						</center>
					</form>
			</div>
		</div>
	...
	<div class="col-sm-12">
		<div class="panel panel-default">
			...
			<div class="col-md-11">
				<label>Un tipo de contribuyente es el elemento principal en el servicio de transporte por lo tanto es un riegistro obligatorio que debe estar disponible en el registro del cliente</label>
			</div> 
		</div>
	</div> 
</div>
...
<script language="javascript">
	...
	function finicio()
	{
		lof=document.fmaestro_tipo_contri;
		
			if (lof.txtoperacion.value=="buscar")
			{
			lof=document.fmaestro_tipo_contri;
			lof.txtdesc_tipo_cont.disabled=true;
	...
	function fnuevo()
	{
		...
		lof.txtdesc_tipo_cont.disabled=false;
		lof.txtdesc_tipo_cont.focus();
		...
		function feliminar()
	{
		lof=document.fmaestro_tipo_contri;
			...
			{
				lof.txtidtipo_contribuyente.disabled=false;
	...
	var miPopup
		...
		miPopup = window.open("ayuda/ayudatipo_contri.php","solicitud","width=450px,height=870px,scrollbars=yes,toolbar=No") //~ aqui se debe colocar la direccion del archivo de la ayuda
		miPopup.focus()
	}
</script>
