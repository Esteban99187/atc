<?php
	...
	********************************************************************************************/
	...
	$liidtipo_cuenta = isset($_GET['liidtipo_cuenta']) ? $_GET['liidtipo_cuenta'] : null;
	$lsdesc_tipo_cuen = isset($_GET['lsdesc_tipo_cuen']) ? $_GET['lsdesc_tipo_cuen'] : null;
	$lihay = isset($_GET['lihay']) ? $_GET['lihay'] : null;
	$lshacer = isset($_GET['lshacer']) ? $_GET['lshacer'] : null;
	$lsestatus_tipcue= isset($_GET['lsestatus_tipcue']) ? $_GET['lsestatus_tipcue'] : null ;
	$lsoperacion = isset($_GET['lsoperacion']) ? $_GET['lsoperacion'] : null;
?>
<script type="text/javascript">
	function BuscarAjaxtipo_cuenta(valor){
		...
		if(document.getElementById('est1').checked){ //si esta tildado la caja de activos
			...
			ajax.send("operacion=BusquedaAjaxtipo_cuenta&status=1&estado="+valor); //paso variable con estatus activo(1) y nombre

		}else if(document.getElementById('est2').checked){ //si esta tildado la caja de inactivos

			ajax.send("operacion=BusquedaAjaxtipo_cuenta&status=0&estado="+valor); //paso variable con estatus inactivo(0) y nombre

		}else{ //si no esta tildados ni activos ni inactivos
			ajax.send("operacion=BusquedaAjaxtipo_cuenta&status=Null&estado="+valor); //paso variable con estatus Nulo y nombre
		}	
	}
</script>
<div class="row">
	<div class="col-sm-12">
		<div class="panel panel-default">
			<div class="panel-heading"> 
				<div class="panel-title">Tipo Cuenta</div> 
				<div class="panel-options">  <a href="#" data-rel="collapse"><i class="entypo-down-open"></i></a> <a href="#" data-rel="reload"><i class="entypo-arrows-ccw"></i></a> <a href="#" data-rel="close"><i class="entypo-cancel"></i></a> <i class="entypo-help" onclick="abretipo_cuenta()"></i></div> <!-- agregar este linea depues del icono close (despues del ultimo </a>) y cambiar el nombre de la funcion onclick<i class="entypo-help" onclick="abreproducto()"></i> luego al final dentro del <script> colocar la funcion onclick -> revisar el final para ver funcion onclick-->
			</div>
				<div class="panel-body">
					<form method="post" name="fmaestro_tipo_cuenta" id="fmaestro_tipo_cuenta" action="../controlador/control_maestro_tipo_cuenta.php">
						...
						<div class="row">
							<div class="form-group col-lg-3"></div>
								<input class="form-control"  type="hidden" name="txtidtipo_cuenta" onkeypress="return soloNumero(event)" title="ingrese ncodigo tipo de cuenta"  maxlength="3" tabindex="1" value="<?php print($liidtipo_cuenta);?>" onblur="fperderfocusidtipo_cuenta();">
								<div class="col-lg-6">
								<div class="form-group has-error">
								<label >Nombre:</label>
								<input class="form-control"  type="text" onBlur="cambio_mayus(this)"  name="txtdesc_tipo_cuen" placeholder="Solo Ingrese Letras"  onkeypress="return soloLetra(event)" title="ingrese nombre tipo de cuenta" maxlength="50" value="<?php print($lsdesc_tipo_cuen);?>"> 
							</div>
							</div>
						...
						<div class="row">
							...
							?>
							<button type="reset"  name="btncancelar"  class="btn btn-lg btn-primary" title="Clic para Cancelar"  onclick="cancelar('maestro_tipo_cuenta')">Cancelar</button>
						</center>
					</form>
			</div>
		</div>
	...
	<div class="col-sm-12">
		<div class="panel panel-default">
			...
			<div class="col-md-11">
				<label>Un tipo de cuenta es el elemento principal en este servicio. Por lo tanto, es un riegistro obligatorio que debe estar disponible en el registro del cliente.</label>
			</div> 
		</div>
	</div> 
</div>
...
<script language="javascript">
	...
	function finicio()
	{
		lof=document.fmaestro_tipo_cuenta;
		
			if (lof.txtoperacion.value=="buscar")
			{
			lof=document.fmaestro_tipo_cuenta;
			lof.txtdesc_tipo_cuen.disabled=true;
		...
		function fnuevo()
	{
		...
		lof.txtdesc_tipo_cuen.disabled=false;
		lof.txtdesc_tipo_cuen.focus();
		...
		function feliminar()
	{
		lof=document.fmaestro_tipo_cuenta;
			...
			{
				lof.txtidtipo_cuenta.disabled=false;
	...
	var miPopup
		function abretipo_cuenta(){ //~ luego de 'funcion' se le coloca el nombre que se le dio en el onclick de arriba
		miPopup = window.open("ayuda/ayudatipo_cuenta.php","solicitud","width=450px,height=870px,scrollbars=yes,toolbar=No") //~ aqui se debe colocar la direccion del archivo de la ayuda
		miPopup.focus()
	}
</script>
