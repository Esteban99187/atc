...
function ponPrefijo(cod, nomco){ 
	opener.document.form1.txtidregion.value = cod;
	opener.document.form1.txtdesc_regi.value = nomco;
...
<h1><?php 
				include("../../modelo/class_region.php");
				$objEquipo = new class_region();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadoregion($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
      ...
      <tr>
        ...
        <th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
			  ...
			  <h4 style="color:#DB4D4D;"> Para seleccionar una Región, haga click en el código.</h4>
		</div>
