...
<script> 
function ponPrefijo(cod, nomco,uni){ 
	opener.document.form1.txtidcapacidad.value = cod;
	opener.document.form1.txtcapacidad.value = nomco;
	opener.document.form1.cmbidunidad_medida.value = uni;
...
<h1><?php 
				include("../../modelo/class_capacidad.php");
				$objEquipo = new class_capacidad();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadocapacidad1($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
      ...
      <tr>
        <?php
						if ($lista<0)
							...
							?>
					<th>
					  <input name="textfield"  class="form-control" type="text"value="<?php echo $codig ?>" size="60" onClick="ponPrefijo(this.value, '<?php echo $nomb?>', '<?php echo $uni?>')"></th>
        <th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
      </tr>
    </tbody> 
		</table>
			  <h4 style="color:#DB4D4D;"> Para seleccionar capacidad, haga click en el codigo.</h4>
		</div>
