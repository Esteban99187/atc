...
function ponPrefijo(cod, nomco){ 
	opener.document.fmaestro_roles.txtcod_rol.value = cod;
	opener.document.fmaestro_roles.txtnombre_rol.value = nomco;
...
<h1><?php 
				include("../../modelo/class_roles.php");
				$objEquipo = new class_roles();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadorol($cod,$nombre);
				if($lista=="99")			
				{	
				?>
				<script> 
    ...
    <tbody>
      <tr>
          ...
          <?php }?>
			  ...
			  <h4 style="color:#DB4D4D;"> Para seleccionar roles, haga click en el codigo.</h4>
		</div>
