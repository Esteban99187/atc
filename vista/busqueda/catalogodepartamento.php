...
<script> 
function ponPrefijo(cod, nomco,tele_depa){ 
	opener.document.fmaestro_departamento.txtiddepartamento.value = cod;
	opener.document.fmaestro_departamento.txtdesc_depa.value = nomco;
	opener.document.fmaestro_departamento.txttele_depa.value = tele_depa;
	window.close()	
} 
</script> 
<style type="text/css">
...
<h1><?php 
				include("../../modelo/class_departamento.php");
				$objEquipo = new class_departamento();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadodepartamento($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
      ...
      <tr>
					...
					<?php
						if ($lista<0)
							...
							?>
					<th>
					  <input name="textfield"  type="text"value="<?php echo $codig ?>" size="10" onClick="ponPrefijo(this.value, '<?php echo $nomb?>', '<?php echo $tele_depa?>')"></th>
