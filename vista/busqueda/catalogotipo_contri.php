...
function ponPrefijo(cod, nomco){ 
	opener.document.fmaestro_tipo_contri.txtidtipo_contribuyente.value = cod;
	opener.document.fmaestro_tipo_contri.txtdesc_tipo_cont.value = nomco;
...
<h1><?php 
				include("../../modelo/class_tipo_contri.php");
				$objEquipo = new class_tipo_contri();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadotipo_contri($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
    ...
    <thead>
      <tr>
        <th>Código</th>
        <th>Nombre del Tipo Contribuyente</th>
      </tr>
    </thead>
      ...
      <tr>
        ...
        <th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
			  ...
			  <h4 style="color:#DB4D4D;"> Para seleccionar tipo contribuyente, haga click en el codigo.</h4>
		</div>
