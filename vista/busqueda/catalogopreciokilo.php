...
function ponPrefijo(cod, nomco,uni){ 
	opener.document.fprecio.txtidprecio.value = cod;
	opener.document.fprecio.txtprecio_pre.value = nomco;
	opener.document.fprecio.cmbidtipo_unidad.value = uni;
...
<h1><?php 
				...
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadopreciokilo($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
      ...
      <tr>
        ...
        <th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
			  ...
			  <h4 style="color:#DB4D4D;"> Para seleccionar un precio, haga click en el código.</h4>
		</div>
