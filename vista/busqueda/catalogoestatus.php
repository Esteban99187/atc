...
function ponPrefijo(cod, nomco){ 
	opener.document.fmaestro_estatus.txtidestatus.value = cod;
	opener.document.fmaestro_estatus.txtnombre_est.value = nomco;
...
<h1><?php 
				include("../../modelo/class_estatus.php");
				$objEquipo = new class_estatus();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadoestatus($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
      ...
      <tr>
        ...
        <th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
			  ...
			  <h4 style="color:#DB4D4D;"> Para seleccionar forma de pago, haga click en el codigo.</h4>
		</div>
