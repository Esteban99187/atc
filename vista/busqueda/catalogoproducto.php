...
function ponPrefijo(cod, nomco, modelo, repre, unid){ 
	opener.document.fproducto.txtidproducto.value = cod;
	opener.document.fproducto.txtdesc_prod.value = nomco;
	opener.document.fproducto.cmbidtipo_producto.value = modelo;
	opener.document.fproducto.cmbidtipo_unidad.value = repre;
	opener.document.fproducto.cmbidunidad_medida.value = unid;
	window.close()	
} 
</script> 
<style type="text/css">
...
<h1><?php 
				...
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadoproducto($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
      ...
      <tr>
					...
					<?php
						if ($lista<0)
							...
							?>
					<th>
					  <input name="textfield"  type="text"value="<?php echo $codig ?>" size="10" onClick="ponPrefijo(this.value, '<?php echo $nomb?>', '<?php echo $modelo?>', '<?php echo $repre?>', '<?php echo $unid?>')">
						<th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
      </tr>
    </tbody> 
		</table>
			  <h4 style="color:#DB4D4D;"> Para seleccionar producto, haga click en el codigo.</h4>
		</div>
