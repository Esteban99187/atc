...
<html>
  <head>
    ...
    <title>ATCSISTEM</title>
      <script>
    function ponPrefijo(cod,nomco,modelo,placa,ob,repre,uni)
    {
    	opener.document.funidad.txtidunidad.value = cod;
    	opener.document.funidad.txtserial_motor.value = nomco;
    	opener.document.funidad.txtserial_carroceria.value = modelo;
    	opener.document.funidad.txtplaca.value = placa;
    	opener.document.funidad.txtobservaciones.value = ob;
    	opener.document.funidad.cmbidmodelo_unidad.value = repre;
    	opener.document.funidad.cmbidtipo_unidad.value = uni;
    	window.close()
    }
    </script>
<style type="text/css">
...
background: linear-gradient(to bottom, rgba(216,96,96,0.13) 0%,rgba(216,96,96,0.13) 100%); /* W3C */ filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#21d86060', endColorstr='#21d86060',GradientType=0 ); /* IE6-9 */"> 
     <h1> <?php
    				...
    				include("../../modelo/class_unidad.php");
    				$objEquipo = new class_unidad();
    				$cod=$_GET["id1"];
    				$nombre=$_GET["id2"];
    				$lista    = $objEquipo->flistadounidad($cod,$nombre);
    				if($lista=="99")
    				{
    	?>
    	<script> alert("No se han encontrado resultados en la busqueda."); </script>
      ...
      <tr>
            <?php
  							...
  							for($i=0;$i<count($lista);$i++)
                {
  					...
  					?>
  					<th>
  					  <input name="textfield"  type="text" value="<?php echo $codig ?>" size="10" onClick="ponPrefijo(this.value, '<?php echo $nomb?>', '<?php echo $modelo?>', '<?php echo $placa?>', '<?php echo $ob?>', '<?php echo $repre?>', '<?php echo $uni?>')"></th>
  						<th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
      </tr>
    </tbody> 
		</table>
			  <h4 style="color:#DB4D4D;"> Para seleccionar unidad, haga click en el codigo.</h4>
		</div>
