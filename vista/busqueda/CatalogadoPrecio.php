	...
	background: linear-gradient(to bottom, rgba(216,96,96,0.13) 0%,rgba(216,96,96,0.13) 100%); /* W3C */ filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#21d86060', endColorstr='#21d86060',GradientType=0 ); /* IE6-9 */"> 
		<h1>
			<?php 
				...
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->FBuscaPrecio($cod,$nombre);
				if($lista=="99")
				{
			?>
			<script> 
		...
		<div class="row spa">
			<div class="col-md-5">
				<div class="panel panel-default">
					<div class="panel-heading"><a href="CatalogoPrecio.php"><strong><i class="glyphicon glyphicon-align-justify"></i>Catalogo de Precio</strong></a>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="BuscaPrecio.php"><strong><i class="glyphicon glyphicon-folder-open"></i>Busqueda de Precio</strong></a></div>
						<div class="panel-body">
								...
								<tbody>
									<tr>
										<?php
											...
											$capacidad = $lista[$i][4];
										?>
										<th><input name="textfield"  class="form-control" type="text"           value="<?php echo $codig ?>"  onClick="ponPrefijo(this.value, '<?php echo $nomb?>', '<?php echo $modelo?>', '<?php echo $capacidad?>')"></th>
										<th><input name="textfield4" class="form-control" type="text"  readonly value="<?php echo $modelo?>"></th>
										<th><input name="textfield1" class="form-control" type="text"  readonly value="<?php echo $capacidad?>"></th>
							...
							<h4 style="color:#DB4D4D;"> Para seleccionar un Precio, haga click en el Codigo.</h4>
						</div>
					</div>
				</div>
			</div>
