...
<html> 
	<head> 
		...
		<script> 
			...
			{ 
				opener.document.freluni.txtidremolque.value = cod;
				opener.document.freluni.txtplaca_rem.value = nomb;
				opener.document.freluni.txtnombre_marrem.value = modelo;
				opener.document.freluni.txtnombre_tiprem.value = ciudad_origen_vto;
				window.close()
			} 
		</script> 
		<style type="text/css">
	...
	background: linear-gradient(to bottom, rgba(216,96,96,0.13) 0%,rgba(216,96,96,0.13) 100%); /* W3C */ filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#21d86060', endColorstr='#21d86060',GradientType=0 ); /* IE6-9 */"> 
		<h1>
			<?php 
				...
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->BuscaRemolque($cod,$nombre);
				if($lista=="99")
				{
			?>
			<script> 
		...
		<div class="row spa">
			<div class="col-md-5">
				<div class="panel panel-default">
					<div class="panel-heading"><a href="CatalogoRemolqueReluni.php"><strong><i class="glyphicon glyphicon-align-justify"></i>Catalogo de Remolque</strong></a>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="BuscaRemolqueReluni.php"><strong><i class="glyphicon glyphicon-folder-open"></i>Busqueda de Remolque</strong></a></div>
						<div class="panel-body">
							...
							<h4 style="color:#DB4D4D;"> Para seleccionar un de Remolque de Transporte, haga click en el Numero.</h4>
						</div>
					</div>
				</div>
			</div>
