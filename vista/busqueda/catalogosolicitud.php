...
<script> 
function ponPrefijo(cod, nomco, modelo, tippro, unimed, pesso, nomed, disa, dien, fesa, feen){ 
	opener.document.form1.txtidsolicitud.value = cod;
...
<h1><?php 
				include("../../modelo/class_solicitud.php");
				$objEquipo = new class_solicitud();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadosolicitud($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
			...
			<fieldset style="width:470;background:#F7F7F7;" id="formu">
			 <form name=fprefijos>
			  <table border="0"  rules="rows">
			   ...
			   <tr>
					<td><p style="font-family:arial;">Código solicitud:</p></td>
				...
				<tr>
					...
					<?php
						if ($lista<0)
							...
							?>
					<td><div align="center">
					  <input name="textfield" type="text"value="<?php echo $codig ?>" size="60" onClick="ponPrefijo(this.value, '<?php echo $nomb?>', '<?php echo $modelo?>', '<?php echo $tippro?>', '<?php echo $unimed?>', '<?php echo $pesso?>', '<?php echo $nomed?>', '<?php echo $disa?>', '<?php echo $dien?>', '<?php echo $fesa?>', '<?php echo $feen?>')">
						</div>
					</td>
					<td><div align="center" >
