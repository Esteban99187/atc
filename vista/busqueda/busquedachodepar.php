...
<legend class="label" style="margin:30px 0px 15px 320px;">Buscar Reportes por Departamento</legend>
<form name="form1" action="">
   ...
   <?   
                                        include_once("modelo/class_departamento.php");
                                        $departamento = new departamento;
                                        $departamento->listar();
                                        echo '<select class="combo" style="width:180px;height:30px;margin:0px 0px 0px 310px;" name="busquedacam" id="cam_iddepartamento">'; 
                                        echo '<option value="-">Seleccione</option>';
                                        while($row_departamento = $departamento->row()){
                                            echo '<option value='.$row_departamento['iddepartamento'];    
                                            if($row_departamento['iddepartamento']== $row_persona['iddepartamento']) echo " selected ";                                 
                                            echo '>'.$row_departamento['1'].'</option>';
                                        }
                                        echo '</select>';
                                    ?>
        ...
        function enviar(){
         var buscar=f.busquedacam.value;
            if(f.busquedacam.value!="-"){
                window.location=("rep_pdf/pdfRep_Chodepar.php?lsa="+buscar);   
            }
            else{ alert("Vacio");
				}
        }
