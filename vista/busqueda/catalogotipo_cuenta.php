...
function ponPrefijo(cod, nomco){ 
	opener.document.fmaestro_tipo_cuenta.txtidtipo_cuenta.value = cod;
	opener.document.fmaestro_tipo_cuenta.txtdesc_tipo_cuen.value = nomco;
...
<h1><?php 
				include("../../modelo/class_tipo_cuenta.php");
				$objEquipo = new class_tipo_cuenta();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadotipo_cuenta($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
      ...
      <tr>
        ...
        <th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
			  ...
			  <h4 style="color:#DB4D4D;"> Para seleccionar tipo de cuenta, haga click en el codigo.</h4>
		</div>
