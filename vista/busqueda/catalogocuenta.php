...
<script> 
function ponPrefijo(cod, nomco, serv, cli){ 
	opener.document.fcuenta.txtidcuenta.value = cod;
	opener.document.fcuenta.cmbbanco_idbanco.value = nomco;
	opener.document.fcuenta.cmbtipo_cuenta_idtipo_cuenta.value = serv;
	opener.document.fcuenta.cmbidcliente.value = cli;
...
<h1><?php 
				include("../../modelo/class_cuenta.php");
				$objEquipo = new class_cuenta();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadocuenta_banco($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
      ...
      <tr>
					...
					<?php
						if ($lista<0)
							...
							?>
					<th>
					  <input name="textfield"  type="text"value="<?php echo $codig ?>" size="10" onClick="ponPrefijo(this.value, '<?php echo $nomb?>', '<?php echo $serv?>', '<?php echo $cli?>')"></th>
						<th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
      </tr>
    </tbody> 
		</table>
			  <h4 style="color:#DB4D4D;"> Para seleccionar cuenta, haga click en el codigo.</h4>
		</div>
