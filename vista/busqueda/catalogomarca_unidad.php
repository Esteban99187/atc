...
function ponPrefijo(cod, nomco){ 
	opener.document.fmaestro_marca_unidad.txtidmarca_unidad.value = cod;
	opener.document.fmaestro_marca_unidad.txtdesc_marc.value = nomco;
...
<h1><?php 
				include("../../modelo/class_marca_unidad.php");
				$objEquipo = new class_marca_unidad();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadomarca_unidad($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
    ...
    <thead>
      <tr>
        <th>Código</th>
        <th>Nombre Marca de la Unidad </th>
      </tr>
    </thead>
      ...
      <tr>
        ...
        <th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
			  ...
			  <h4 style="color:#DB4D4D;"> Para seleccionar marca de la unidad, haga click en el codigo.</h4>
		</div>
