...
function ponPrefijo(cod, nomco){ 
	opener.document.form1.txtidpais.value = cod;
	opener.document.form1.txtdesc_pais.value = nomco;
	window.close()	
} 
</script> 
<style type="text/css">
...
<h1><?php 
				include("../../modelo/class_pais.php");
				$objEquipo = new class_pais();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadopais($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
    ...
    <tbody>
      <tr>
          ...
          <?php }?>
			  ...
			  <h4 style="color:#DB4D4D;"> Para seleccionar un Pais, haga click en el código.</h4>
		</div>
