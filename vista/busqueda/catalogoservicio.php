...
<script> 
function ponPrefijo(cod, nomco, serv){ 
	opener.document.fmaestro_servicio.txtidservicio.value = cod;
	opener.document.fmaestro_servicio.txtdescripcion_ser.value = nomco;
	opener.document.fmaestro_servicio.cmbidtipo_servicio_ser.value = serv;
...
<h1><?php 
				include("../../modelo/class_servicio.php");
				$objEquipo = new class_servicio();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadoservicio($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
      ...
      <tr>
					...
					<?php
						if ($lista<0)
							...
							?>
					<th>
					  <input name="textfield"  type="text"value="<?php echo $codig ?>" size="20" onClick="ponPrefijo(this.value, '<?php echo $nomb?>', '<?php echo $serv?>')"></th>
						<th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
      </tr>
    </tbody> 
		</table>
			  <h4 style="color:#DB4D4D;"> Para seleccionar  servicio, haga click en el código.</h4>
		</div>
