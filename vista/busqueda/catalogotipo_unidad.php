...
function ponPrefijo(cod, nomco, modelo){ 
	opener.document.fmaestro_tipo_unidad.txtidtipo_unidad.value = cod;
	opener.document.fmaestro_tipo_unidad.txtdesc_tipo_unid.value = nomco;
	opener.document.fmaestro_tipo_unidad.cmbidcapacidad.value = modelo;
	window.close()	
} 
</script> 
<style type="text/css">
...
<h1><?php 
				...
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadotipo_unidad($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
    ...
    <thead>
      <tr>
        <th>Código</th>
        <th>nombre de tipo de unidad</th>
      </tr>
    </thead>
    ...
    <tbody>
      <tr>
          ...
          <?php }?>
			  ...
			  <h4 style="color:#DB4D4D;"> Para seleccionar tipo unidad, haga click en el codigo.</h4>
		</div>
