...
function ponPrefijo(cod, nomco){ 
	opener.document.fmaestro_tipo_servicio.txtidtipo_servicio.value = cod;
	opener.document.fmaestro_tipo_servicio.txtdescripcion_tipser.value = nomco;
...
<h1><?php 
				include("../../modelo/class_tipo_servicio.php");
				$objEquipo = new class_tipo_servicio();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadotipo_servicio($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
    ...
    <tbody>
      <tr>
          ...
          <?php }?>
			  ...
			  <h4 style="color:#DB4D4D;"> Para seleccionar Tipo de servicio, haga click en el codigo.</h4>
		</div>
