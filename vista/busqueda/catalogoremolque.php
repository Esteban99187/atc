...
<script> 
function ponPrefijo(cod, nomco, modelo, repre, ob, uni, tip){ 
	opener.document.form1.txtidremolque.value = cod;
	opener.document.form1.txtserial_rem.value = nomco;
	opener.document.form1.txtserial_carroseria_rem.value = modelo;
	opener.document.form1.txtplaca_rem.value = repre;
	opener.document.form1.txtobservaciones_rem.value = ob;
	opener.document.form1.cmbidmodelo_unidad_rem.value = uni;
	opener.document.form1.cmbidtipo_unidad_rem.value = tip;
	window.close()	
} 
</script> 
<style type="text/css">
...
<h1><?php 
				include("../../modelo/class_remolque.php");
				$objEquipo = new class_remolque();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadoremolque($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
      ...
      <tr>
					...
					<?php
						if ($lista<0)
							...
							?>
					<th>
					  <input name="textfield"  type="text"value="<?php echo $codig ?>" size="10" onClick="ponPrefijo(this.value, '<?php echo $nomb?>', '<?php echo $modelo?>', '<?php echo $repre?>', '<?php echo $ob?>', '<?php echo $uni?>', '<?php echo $tip?>')"></th>
						<th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
      </tr>
    </tbody> 
		</table>
			  <h4 style="color:#DB4D4D;"> Para seleccionar remolque, haga click en el codigo.</h4>
		</div>
