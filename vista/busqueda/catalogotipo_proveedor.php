...
function ponPrefijo(cod, nomco){ 
	opener.document.fmaestro_tipo_proveedor.txtidtipo_proveedor.value = cod;
	opener.document.fmaestro_tipo_proveedor.txtdesc_tipo_prov.value = nomco;
	window.close()	
} 
</script> 
<style type="text/css">
...
<h1><?php 
				include("../../modelo/class_tipo_proveedor.php");
				$objEquipo = new class_tipo_proveedor();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadotipo_proveedor($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
    ...
    <thead>
      <tr>
        <th>Código</th>
        <th>nombre de tipo proveedor</th>
      </tr>
    </thead>
    ...
    <tbody>
      <tr>
          ...
          <?php }?>
			  ...
			  <h4 style="color:#DB4D4D;"> Para seleccionar tipo proveedor, haga click en el codigo.</h4>
		</div>
