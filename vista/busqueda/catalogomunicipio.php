...
<script> 
function ponPrefijo(cod, nomco, modelo, pa){ 
	opener.document.form1.txtidmunicipio.value = cod;
	opener.document.form1.txtdesc_muni.value = nomco;
	opener.document.form1.cmbidestado.value = modelo;	
	opener.document.form1.cmbidpais.value = pa;	
	window.close()	
} 
</script> 
<style type="text/css">
...
<h1><?php 
				...
				$objEquipo = new class_municipio();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadomunicipio($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
      ...
      <tr>
					...
					<?php
						if ($lista<0)
							...
							?>
					<th>
					  <input name="textfield"  type="text"value="<?php echo $codig ?>" size="10" onClick="ponPrefijo(this.value, '<?php echo $nomb?>', '<?php echo $modelo?>', '<?php echo $pa?>')"></th>
						<th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
      </tr>
    </tbody> 
		</table>
			  <h4 style="color:#DB4D4D;"> Para seleccionar municipio, haga click en el codigo.</h4>
		</div>
