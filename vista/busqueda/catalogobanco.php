...
function ponPrefijo(cod, nomco){ 
	opener.document.fbanco.txtidbanco.value = cod;
	opener.document.fbanco.txtdesc_banc.value = nomco;
...
<h1><?php 
				include("../../modelo/class_banco.php");
				$objEquipo = new class_banco();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadobanco($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
      ...
      <tr>
        ...
        <th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
			  ...
			  <h4 style="color:#DB4D4D;"> Para seleccionar banco, haga click en el codigo.</h4>
		</div>
