...
function ponPrefijo(cod, nomco){ 
	opener.document.fmaestro_tipo_cliente.txtidtipo_cliente.value = cod;
	opener.document.fmaestro_tipo_cliente.txtdesc_tipo_clie.value = nomco;
...
<h1><?php 
				include("../../modelo/class_tipo_cliente.php");
				$objEquipo = new class_tipo_cliente();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadotipo_cliente($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
    ...
    <tbody>
      <tr>
          ...
          <?php }?>
			  ...
			  <h4 style="color:#DB4D4D;"> Para seleccionar tipo de cliente, haga click en el codigo.</h4>
		</div>
