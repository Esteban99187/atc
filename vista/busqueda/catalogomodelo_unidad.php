...
<script> 
function ponPrefijo(cod, nomco,ano,marca){ 
	opener.document.fmaestro_modelo_unidad.txtidmodelo_unidad.value = cod;
	opener.document.fmaestro_modelo_unidad.txtdesc_mode.value = nomco;
	opener.document.fmaestro_modelo_unidad.txtano_mode.value = ano;
	opener.document.fmaestro_modelo_unidad.cmbidmarca_unidad.value = marca;
...
<h1><?php 
				include("../../modelo/class_modelo_unidad.php");
				$objEquipo = new class_modelo_unidad();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadomodelo_unidad($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
      ...
      <tr>
        <?php
						if ($lista<0)
							...
							?>
					<th>
					  <input name="textfield"  class="form-control" type="text"value="<?php echo $codig ?>" size="60" onClick="ponPrefijo(this.value, '<?php echo $nomb?>', '<?php echo $ano?>', '<?php echo $marca?>')"></th>
        <th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
      </tr>
    </tbody> 
		</table>
			  <h4 style="color:#DB4D4D;"> Para seleccionar modelo de la unidad, haga click en el codigo.</h4>
		</div>
