...
function ponPrefijo(cod, nomco){ 
	opener.document.form1.txtidciudad.value = cod;
	opener.document.form1.txtdesc_ciud.value = nombre;
	opener.document.form1.txtidparroquia.value = parro;
	opener.document.form1.txtidmunicipio.value = muni;
	opener.document.form1.txtidestado.value = esta;
	opener.document.form1.txtidpais.value = pais;
	opener.document.form1.txtcodi_posta.value = codpos;
...
<h1><?php 
				include("../../modelo/class_ciudad.php");
				$objEquipo = new class_ciudad();
				$cod=$_GET["id1"]; 
				$nomb=$_GET["id2"]; 
			
				$lista    = $objEquipo->flistadociudad($cod,$nomb);
				if($lista=="99")				
				{	
				?>
				<script> 
      ...
      <tr>
        <?php
						if ($lista<0)
							...
							?>
					<th>
					  <input name="textfield"  class="form-control" type="text"value="<?php echo $codig ?>" size="60" onClick="ponPrefijo(this.value, '<?php echo $nomb?>')"></th>
        <th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
      </tr>
    </tbody> 
		</table>
			  <h4 style="color:#DB4D4D;"> Para seleccionar una Ciudad, haga click en el codigo.</h4>
		</div>
