...
function ponPrefijo(cod, nomco){ 
	opener.document.ftipo_producto.txtidtipo_producto.value = cod;
	opener.document.ftipo_producto.txtdesc_tipo_prod.value = nomco;
...
<h1><?php 
				include("../../modelo/class_tipo_producto.php");
				$objEquipo = new class_tipo_producto();
				$cod=$_GET["id1"]; 
				$nombre=$_GET["id2"]; 
				$lista    = $objEquipo->flistadotipo_producto($cod,$nombre);
				if($lista=="99")				
				{	
				?>
				<script> 
      ...
      <tr>
        ...
        <th><input name="textfield2"  type="text" class="form-control" size="150" readonly value="<?php echo $nomb?>" size="15"></th>
          <?php }?>
			  ...
			  <h4 style="color:#DB4D4D;"> Para seleccionar tipo producto, haga click en el codigo.</h4>
		</div>
