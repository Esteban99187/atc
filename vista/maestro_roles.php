...
$licod_rol = isset($_GET['licod_rol']) ? $_GET['licod_rol'] : null;
$lsnombre_rol = isset($_GET['lsnombre_rol']) ? $_GET['lsnombre_rol'] : null;
$lihay = isset($_GET['lihay']) ? $_GET['lihay'] : null;
$lshacer = isset($_GET['lshacer']) ? $_GET['lshacer'] : null;
$lsestatus_rol = isset($_GET['lsestatus_rol']) ? $_GET['lsestatus_rol'] : null;
$lsoperacion = isset($_GET['lsoperacion']) ? $_GET['lsoperacion'] : null;

?>
<script type="text/javascript">
	function BuscarAjaxroles(valor){
		...
		if(document.getElementById('est1').checked){ //si esta tildado la caja de activos
			...
			ajax.send("operacion=BusquedaAjaxroles&status=1&estado="+valor); //paso variable con estatus activo(1) y nombre

		}else if(document.getElementById('est2').checked){ //si esta tildado la caja de inactivos

			ajax.send("operacion=BusquedaAjaxroles&status=0&estado="+valor); //paso variable con estatus inactivo(0) y nombre

		}else{ //si no esta tildados ni activos ni inactivos
			ajax.send("operacion=BusquedaAjaxroles&status=Null&estado="+valor); //paso variable con estatus Nulo y nombre
		}	
	}
</script>
<div class="row">
	<div class="col-sm-12">
		<div class="panel panel-default">
			<div class="panel-heading"> 
				...
				<div class="panel-options">  <a href="#" data-rel="collapse"><i class="entypo-down-open"></i></a> <a href="#" data-rel="reload"><i class="entypo-arrows-ccw"></i></a> <a href="#" data-rel="close"><i class="entypo-cancel"></i></a> <i class="entypo-help" onclick="abreroles()"></i></div> <!-- agregar este linea depues del icono close (despues del ultimo </a>) y cambiar el nombre de la funcion onclick<i class="entypo-help" onclick="abreproducto()"></i> luego al final dentro del <script> colocar la funcion onclick -> revisar el final para ver funcion onclick-->
			</div>
				<div class="panel-body">
					<form method="post" name="fmaestro_roles" id="fmaestro_roles" action="../controlador/control_maestro_roles.php">
						...
						<div class="row">
							<div class="form-group col-lg-4"></div>
								<input class="form-control"  type="hidden" name="txtcod_rol" onkeypress="return soloNumero(event)" title="ingrese codigo del cargo"  maxlength="3" tabindex="1" value="<?php print($licod_rol);?>" onblur="fperderfocuscod_rol();">
								<div class="col-lg-4">
								<div class="form-group has-error">
								<label >Nombre:</label>
								<input class="form-control" onBlur="cambio_mayus(this)" placeholder="Solo Ingrese Letras" type="text" name="txtnombre_rol" onkeypress="return soloLetra(event)" title="ingrese nombre descriptivo del cargo" maxlength="50"   value="<?php print($lsnombre_rol);?>"> 
							</div>		
							</div>	
						...
						<center>
							...
							?>
							<button type="reset"  name="btncancelar"  class="btn btn-lg btn-primary" title="Clic para Cancelar"  onclick="cancelar('maestro_roles')">Cancelar</button>
						</center>
					</form>
			</div>
		</div> 
...
<div class="row">
	<div class="col-sm-12">
		<div class="panel panel-default">
			...
			<div class="col-md-11">
				<label>Este permite registrar cada uno de los cargos encontrados en cada departamentos de la empresa. </label>
			</div> 
		</div>
	</div> 
</div>
...
<script language="javascript">
	...
	function finicio()
	{
		lof=document.fmaestro_roles;
		
			if (lof.txtoperacion.value=="buscar")
			{
			lof=document.fmaestro_roles;
			lof.txtnombre_rol.disabled=true;
	...
	function fnuevo()
	{
		...
		lof.txtnombre_rol.disabled=false;
		lof.txtnombre_rol.focus();
...
//~ Funcion del onclick , este me permite abrir la ventana emergente, tiene que estar declarado igual que en la parte de arrriba
	var miPopup
		function abreroles(){ //~ luego de 'funcion' se le coloca el nombre que se le dio en el onclick de arriba
		miPopup = window.open("ayuda/ayudaroles.php","solicitud","width=450px,height=870px,scrollbars=yes,toolbar=No") //~ aqui se debe colocar la direccion del archivo de la ayuda
		miPopup.focus()
	}
</script>
