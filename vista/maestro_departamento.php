<?php
	...
	********************************************************************************************/
	...
	$liiddepartamento= isset($_GET['liiddepartamento']) ? $_GET['liiddepartamento'] : null ;
	$lsdesc_depa= isset($_GET['lsdesc_depa']) ? $_GET['lsdesc_depa'] : null ;
	$lstele_depa= isset($_GET['lstele_depa']) ? $_GET['lstele_depa'] : null ;
	$lihay= isset($_GET['lihay']) ? $_GET['lihay'] : null ;
	$lshacer= isset($_GET['lshacer']) ? $_GET['lshacer'] : null ;
	$lsestatus_dep= isset($_GET['lsestatus_dep']) ? $_GET['lsestatus_dep'] : null ;
	...
	<div class="col-sm-12">
		<div class="panel panel-default">
			<div class="panel-heading"> 
				...
				<div class="panel-options">  <a href="#" data-rel="collapse"><i class="entypo-down-open"></i></a> <a href="#" data-rel="reload"><i class="entypo-arrows-ccw"></i></a> <a href="#" data-rel="close"><i class="entypo-cancel"></i></a> <i class="entypo-help" onclick="abredepartamento()"></i></div> <!-- agregar este linea depues del icono close (despues del ultimo </a>) y cambiar el nombre de la funcion onclick<i class="entypo-help" onclick="abreproducto()"></i> luego al final dentro del <script> colocar la funcion onclick -> revisar el final para ver funcion onclick-->
			</div>
				<div class="panel-body">
					<form method="post" name="fmaestro_departamento" id="fmaestro_departamento" action="../controlador/control_maestro_departamento.php">
						...
						<div class="row">
							...
							<input class="form-control"  type="hidden" name="txtiddepartamento" title="ingrese nombre del departamento"  maxlength="3" tabindex="1" value="<?php print($liiddepartamento);?>" onblur="fperderfocusiddepartamento();">
							<div class="col-lg-4">
								<div class="form-group has-error">
								<label >Nombre:</label>
								<input class="form-control"  type="text" onBlur="cambio_mayus(this)" placeholder="Solo Ingrese Letras" name="txtdesc_depa" title="ingrese nombre descriptivo del departamento" maxlength="50" onkeypress="return soloLetra(event)" tabindex="2"  value="<?php print($lsdesc_depa);?>">
							</div>	
							</div>	
							<div class="form-group col-lg-1">
								...
								<div class="form-group has-error">
									<label >Teléfono:</label>
										<input class="form-control"  placeholder="Solo Ingrese Números" type="text" name="txttele_depa" title="ingrese telefono del departamento" maxlength="11" onkeypress="return soloNumero(event)" tabindex="2"  value="<?php print($lstele_depa);?>"> 
								</div>		
				</div>	
				<div class="form-group col-lg-1">
								<p style="margin-top:28px;text-left;font-size:30px;color:#C60404;">*</p>
						...
						<center>
							...
							?>
							<button type="reset"  name="btncancelar"  class="btn btn-lg btn-primary" title="Clic para Cancelar"  onclick="cancelar('maestro_departamento')">Cancelar</button>
						</center>
					</form>
			</div>
		</div> 
...
<div class="row">
	<div class="col-sm-12">
		<div class="panel panel-default">
			...
			<div class="col-md-11">
				<label>Registro de los departamentos encontrados en la Empresa </label>
			</div> 
		</div>
	</div> 
</div>
...
<script language="javascript">
	...
	function finicio()
	{
		lof=document.fmaestro_departamento;
		
			if (lof.txtoperacion.value=="buscar")
			{
			lof=document.fmaestro_departamento;
			lof.txtdesc_depa.disabled=true;
			lof.txttele_depa.disabled=true;
	...
	function fnuevo()
	{
		...
		lof.txtdesc_depa.disabled=false;
		lof.txtdesc_depa.focus();
		lof.txttele_depa.disabled=false;
	...
	function feliminar()
	{
		lof=document.fmaestro_departamento;
			...
			{
				lof.txtiddepartamento.disabled=false;
	...
	var miPopup
		function abredepartamento(){ //~ luego de 'funcion' se le coloca el nombre que se le dio en el onclick de arriba
		miPopup = window.open("ayuda/ayudadepartamento.php","solicitud","width=450px,height=870px,scrollbars=yes,toolbar=No") //~ aqui se debe colocar la direccion del archivo de la ayuda
		miPopup.focus()
	}
</script>
