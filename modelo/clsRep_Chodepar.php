<?php
	...
	class clsRep_Chodepar extends db{
		...
		public function fBusqueda(){
			$liI=0;
			if($this->asbusqueda=="-"){
				$lsSql=" select p.cedula, p.nombres_per, p.apellidos_per, p.telefono_corp_per, r.nombre_rol, p.cod_rol, r.cod_rol, d.iddepartamento, d.nombre_dep, p.iddepartamento
												from persona as p
												inner join rol as r
			...
			}else if ($this->asbusqueda!="")
			{
				$lsSql=" select p.cedula, p.nombres_per, p.apellidos_per, p.telefono_corp_per, r.nombre_rol, p.cod_rol, r.cod_rol, d.iddepartamento, d.nombre_dep, p.iddepartamento
												...
												inner join rol as r
												inner join departamento as d
												where (p.cod_rol=r.cod_rol) and (p.iddepartamento=d.iddepartamento) and(p.iddepartamento='$this->asbusqueda')"; 
			}
			$this->fConectar();
